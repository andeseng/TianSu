package com.sunrun.sunrunframwork.weight.ruler;
...
public class HeightView extends View {
     ...
     * 短线的长度
     */
    private float mShortLineLength = 12;
    /**
     * 长线的长度
     */
     ...
     * Unit 单位
     */
    private String mRulerUnitString;
    /**
     * 间距 (整数  3-10)
     */
    private int mRulerSpaceNum = 7;
    /**
     * 刻度上对齐
     */
    public static final int LINE_ALIGN_TOP = 1;
    /**
     * 刻度下对齐
     */
    public static final int LINE_ALIGN_BOTTOM = 2;
    private int lineAlign=LINE_ALIGN_TOP;
    ...
    public HeightView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        if (attrs != null) {
            ...
            lineAlign=attributes.getInt(R.styleable.HeightView_line_align,LINE_ALIGN_TOP);
            ...
            mLineColor = attributes.getColor(R.styleable.HeightView_rulerLineColor, mLineColor);
            mPaint.setColor(mLineColor);

            mLineWidth = attributes.getDimension(R.styleable.HeightView_rulerLineWidth, mLineWidth);
            ...
            mRulerUnitString = attributes.getString(R.styleable.HeightView_RulerUnitString);

            if (mRulerUnitString == null) {
                mRulerUnitString = "";
            }
            mRulerSpaceNum = attributes.getInt(R.styleable.HeightView_RulerSpace, mRulerSpaceNum);
            space = (int) (getResources().getDisplayMetrics().density * mRulerSpaceNum);


            mShortLineLength = attributes.getDimension(R.styleable.HeightView_RulerShortLine, 12);
            mShortLineLength = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, mShortLineLength, getResources().getDisplayMetrics());

            mLongLineLength = attributes.getDimension(R.styleable.HeightView_RulerLongLine, 20);
            mLongLineLength = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, mLongLineLength, getResources().getDisplayMetrics());
        ...
        for (int i = mStartLine, j = mStartLine, k = 0; i <= mLines; i++, k++) {
            ...
            switch (i % mOutSideLine) {
                case ZERO: {
                    ...
                    canvas.drawText(String.valueOf(j++ * mSetupValue) + mRulerUnitString, left + (maxTextWidth - currentTextWidth) / 2, bottom - i * space, mTextPaint);
    ...
    protected void drawHorizontal(Canvas canvas) {
        ...
        if(lineAlign==LINE_ALIGN_BOTTOM){
            //生成线组
          drawHorizontalBottom(canvas);
        }else if(lineAlign==LINE_ALIGN_TOP){
            drawHorizontalTop(canvas);
        }
    }
    protected  void drawHorizontalBottom(Canvas canvas){
        int bottom = getHeight() - getPaddingBottom();
        int left = getPaddingLeft();
        ...
        for (int i = mStartLine, j = mStartLine, k = 0; i <= mLines; i++, k++) {
            ...
            switch (i % mOutSideLine) {
                case ZERO: {
                    canvas.drawText(String.valueOf(j++ * mSetupValue) + mRulerUnitString, left + i * space, bottom, mTextPaint);
                    lineLength = mLongLineLength;
                }
                break;
                default:
            ...
            //drawHorizontalTop(k,left,i,top,lineLength);
        }
    ...
    protected  void drawHorizontalTop(Canvas canvas){
        int bottom = getHeight() - getPaddingBottom();
        int left = getPaddingLeft();
        int top = getPaddingTop();
        for (int i = mStartLine, j = mStartLine, k = 0; i <= mLines; i++, k++) {
            ...
            mLinesArr[k * 4 + 2] = left + i * space;
            /*stopY*/
            mLinesArr[k * 4 + 3] = top + lineLength;
            //drawHorizontalTop(k,left,i,top,lineLength);
        }
        ...
        //绘制当前选中的线条
        canvas.drawLine(left + mCurrentLine * space, ( top ), left + mCurrentLine * space,
                (top ) + (mCurrentLine % mOutSideLine == 0 ? mLongLineLength : mShortLineLength), mHighlightPaint);
    ...
    public float getValue(){
        return mCurrentLine*(mSetupValue/10f);
    }
    public void setValue(float value){
        setCurrentLine((int) (value/(mSetupValue/10f)));
    }

    public void setCurrentLine(int currentLine) {
        currentLine = currentLine - (38 - mRulerSpaceNum * 2);
